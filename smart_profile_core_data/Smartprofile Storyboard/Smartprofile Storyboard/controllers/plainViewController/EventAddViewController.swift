...
//        let event = Event(name: name, address: Address(street: street, city: city, state: state), description: description, start: transformStringToDate(dateString: start)!, end: transformStringToDate(dateString: end)!)
        let event = Event(context: NSManagedObjectContext.current)
        let cdm = CoreDataManager.shared
        event.id = cdm.getNextEventId()
        event.name = name
        event.eventDescription = description
        let address = Address(context: NSManagedObjectContext.current)
        address.street = street
        address.city = city
        address.state = state
        event.address = address
        event.start = transformStringToDate(dateString: start)!
        event.end = transformStringToDate(dateString: end)!
        events[event.id] = event
        do {
//            try cdm.addAddress(address: address)
            try cdm.addEvent(event: event)
        }catch let error as NSError {
            print("❌ error in add event \(error)")
        }
